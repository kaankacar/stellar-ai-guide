...
|---|---|---|
| **Raven** | Official hosted MCP server: Stellar docs + live ecosystem data for any AI agent (`search` + `execute`); replaces the retired Stella bot. Connect the remote MCP to your agent (the /playground is a demo, not a dev workflow) | https://raven.stellar.buzz (`claude mcp add --transport http stellar-raven "https://raven.stellar.buzz/mcp"`) |
